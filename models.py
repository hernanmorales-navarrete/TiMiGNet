...
def save_models_2D(step, g_model_AtoB, g_model_BtoA):
    filename1 = './models/g_model_AtoB_2D_%06d.h5' % (step + 1) # save the first generator model
    g_model_AtoB.save(filename1)
    
    filename2 = './models/g_model_BtoA_2D_%06d.h5' % (step + 1)
...
def summarize_performance_2D(step, g_model, trainX, name, n_samples = 5):
    ...
    # save plot to file
    filename1 = './example_images/%s_2D_generated_plot_%06d.png' % (name, (step + 1))
...
def train_2D(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset, batch_size = 1, save_freq = 10, epochs = 1):
    ...
    try: 
        for i in range(n_steps):
            ...
            if (i + 1) % (bat_per_epo * save_freq) == 0:
                ...
                summarize_performance_2D(i, g_model_AtoB, trainA, 'AtoB')
                # plot B -> A translation
                summarize_performance_2D(i, g_model_BtoA, trainB, 'BtoA')
                # save the models                
                save_models_2D(i, g_model_AtoB, g_model_BtoA)
        ...
        df_losses.to_csv('./LOSSES_TRAINING.csv')
...
def save_models_3D(step, g_model_AtoB, g_model_BtoA):
    filename1 = './models/g_model_AtoB_3D_%06d.h5' % (step + 1) # save the first generator model
    g_model_AtoB.save(filename1)
    
    filename2 = './models/g_model_BtoA_3D_%06d.h5' % (step + 1)
    g_model_BtoA.save(filename2)
    
    print('>Saved: %s and %s' % (filename1, filename2))
      
    
def summarize_performance_3D(step, g_model, trainX, name, n_samples = 1):
    ...
    tiff.imwrite('./example_images/' + name + '_3D_original_' + str(step + 1) + '.tif', X_in)
    tiff.imwrite('./example_images/' + name + '_3D_generated_' + str(step + 1) + '.tif', X_out)
...
def train_3D(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset, batch_size = 1, save_freq = 10, epochs = 1):
...
def patches_2d(original_img, patch_shape, patch_step):
...
def patches_3d(original_img, patch_shape, patch_step):
...
def train_model_2D(dataset, batch_size, epochs, save_freq):
    ...
    train = train_2D(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset, batch_size = batch_size, save_freq = save_freq, epochs = epochs)
...
def train_model_3D(dataset, batch_size, epochs, save_freq):
    ...
    train = train_3D(d_model_A, d_model_B, g_model_AtoB, g_model_BtoA, c_model_AtoB, c_model_BtoA, dataset, batch_size = batch_size, save_freq = save_freq, epochs = epochs)
...
def generate_dataset(model_type, images, patch_shape, patch_step, n_samples, seed_a, seed_b, shuffle_ = True):
    
    data = []
    
    if model_type == '2d':
        for image in images:
            patches = patches_2d(image, patch_shape, patch_step)
            data.append(patches)
        print('total number of patches for image 0: ', len(data[0]))
        print('total number of patches for image 1: ', len(data[1]))
    
    elif model_type == '3d':
        for image in images:
            patches = patches_3d(original_img = image, patch_shape = patch_shape, patch_step = patch_step)
            data.append(patches)
        print('total number of 3D patches for image 0: ', len(data[0]))
        print('total number of 3D patches for image 1: ', len(data[1]), '\n')
    ...
    if shuffle_:
        ...
        print('%d %s patches were loaded for image 0 - SHUFFLED' % (len(data[0]), str(model_type).upper()))
        print('%d %s patches were loaded for image 1 - SHUFFLED' % (len(data[1]), str(model_type).upper()))
        return data
        ...
        print('%d %s patches were loaded for image 0 - NON-SHUFFLED' % (len(data[0]), str(model_type).upper()))
        print('%d %s patches were loaded for image 1 - NON-SHUFFLED' % (len(data[1]), str(model_type).upper()))
                
        return data

def merge_datasets(datasets, seed_a, seed_b, shuffle_ = False):
    data_a = []
    data_b = []
    ...
    if shuffle_:
        return [shuffle(np.concatenate(data_a), random_state = seed_a), shuffle(np.concatenate(data_b), random_state = seed_b)]
    else:
        return [np.concatenate(data_a), np.concatenate(data_b)]
...
def image_postprocessing_norm(prediction):
...
def predict_2D(input_image_path, output_name, model_path, patch_size):
    ...
    pred = image_postprocessing_norm(pred)
    
    tiff.imwrite(output_name + '.tif', pred)
    
    return pred
    

def predict_3D(input_image_path, output_name, model_path, patch_size, overlap_pct = 0.125):
